...
router.beforeEach((to, from, next) => {
  const roles = store.getters && store.getters.roles // Get user roles from the store
  if (to.meta && to.meta.roles) {
    if (roles.some(role => to.meta.roles.includes(role))) {
      next() // User has the required role, allow access
    } else {
      next({ path: '/401', replace: true }) // User does not have the required role, redirect to 401 page
    }
  } else {
